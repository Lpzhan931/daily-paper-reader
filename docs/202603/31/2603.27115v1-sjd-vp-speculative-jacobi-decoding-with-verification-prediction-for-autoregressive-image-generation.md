...
method: 提出SJD-VP方法，通过追踪迭代间标记概率的变化来引导采样，优先选择概率上升的标记以预测验证结果。
result: 实验证明该方法在显著加速自回归解码过程的同时，还提升了生成图像的视觉质量。
conclusion: SJD-VP是一种高效且即插即用的加速方案，通过利用迭代动态优化了自回归图像生成的效率与效果。
---

## 摘要
投机雅可比解码（SJD）已成为加速自回归图像生成的一种极具前景的方法。尽管具有潜力，但由于 Token 选择的歧义性，现有的 SJD 方法通常面临投机 Token 接受率低的问题。近期的工作主要尝试从放宽 Token 验证的角度来缓解这一问题，但未能充分利用解码的迭代动力学。在本文中，我们进行了深入分析，并提出了一个新的观察结果：概率增加的 Token 更有可能与验证通过且正确的 Token 相匹配。基于此，我们提出了一种新颖的带有验证预测的投机雅可比解码（SJD-VP）。其核心思想是利用迭代过程中 Token 概率的变化来引导采样，倾向于选择概率增加的 Token。这有效地预测了哪些 Token 可能通过后续验证，从而提高了接受率。特别地，我们的 SJD-VP 是即插即用的，可以无缝集成到现有的 SJD 方法中。在标准基准测试上的大量实验表明，我们的 SJD-VP 方法在提高图像生成质量的同时，持续加速了自回归解码。
