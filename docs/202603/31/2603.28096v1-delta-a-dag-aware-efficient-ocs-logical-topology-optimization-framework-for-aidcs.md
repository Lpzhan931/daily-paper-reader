...
## 摘要
大语言模型（LLM）的快速扩展加剧了 AI 数据中心（AIDC）中的通信瓶颈。为了克服这一问题，光电路交换（OCS）因其卓越的带宽容量和能效而越来越多地被采用。然而，其重构开销阻碍了迭代内的拓扑更新，因此需要预先设计静态拓扑以吸收随时间变化的 LLM 流量。现有方法基于流量矩阵设计这些拓扑，但这种表示方式掩盖了由并行化策略决定的突发并发带宽需求，且未能考虑并发通信所需的独立通道。为此，我们提出了 DELTA，这是一种面向 AIDC 的高效逻辑拓扑优化框架。它利用计算-通信有向无环图（DAG）将随时间变化的流量模式编码到混合整数线性规划（MILP）模型中，同时利用非关键任务的时间松弛来节省光端口，且不损害迭代完工时间。通过开创性的变长时间间隔公式，DELTA 与固定时间步长公式相比显著缩小了求解空间。为了扩展到千 GPU 集群，我们设计了一种双轨加速策略，将搜索空间剪枝（将复杂度从二次降低到线性）与启发式热启动相结合。对大规模 LLM 工作负载的评估表明，与最先进的基于流量矩阵的基准方法相比，DELTA 将通信时间缩短了高达 17.5%。此外，该框架将光端口消耗降低了至少 20%；将这些剩余端口动态重新分配给带宽受限的工作负载，使其相对于理想无阻塞电网络的性能差距缩小了高达 26.1%，最终使大多数工作负载能够达到近乎理想的性能。
