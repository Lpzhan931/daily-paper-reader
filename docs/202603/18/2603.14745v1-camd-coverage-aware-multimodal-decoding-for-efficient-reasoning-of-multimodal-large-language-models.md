...
tldr: 针对多模态大语言模型在推理中存在的计算资源与任务难度不匹配问题，本文提出了覆盖感知多模态解码（CAMD）。该方法基于重尾难度分布的理论分析，通过动态分配计算资源，结合证据加权评分、后验覆盖估计和序列贝叶斯更新，在有限的Token预算下实现了效率与可靠性的平衡，显著提升了模型在复杂任务上的推理表现。
selection_source: fresh_fetch
motivation: 现有解码方法在简单任务上浪费计算资源而在困难任务上投入不足，导致模型在处理不同难度的多模态推理时效率低下。
method: 提出CAMD框架，通过估计不确定性来动态分配计算量，并结合证据加权评分与序列贝叶斯更新实现自适应推理。
result: 在多个基准数据集上的实验表明，CAMD在有限的计算预算下显著优于现有基准，提升了推理的准确性与效率。
conclusion: 通过覆盖感知的动态解码机制，可以有效解决多模态推理中的计算失配问题，实现模型性能与推理成本的最优平衡。
---
