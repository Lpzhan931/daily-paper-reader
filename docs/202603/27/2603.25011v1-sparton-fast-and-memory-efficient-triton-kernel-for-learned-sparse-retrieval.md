...
method: 提出Sparton算子，利用Triton将矩阵乘法、ReLU、Log1P和最大池化融合进单个内核，并通过对原始逻辑值分块进行早期在线归约来避免内存物化。
result: 相比PyTorch基准，Sparton实现了最高4.8倍的算子加速，并在大词表模型中将训练速度提升2.5倍，支持的批次大小增加26倍。
conclusion: Sparton通过算子融合和内存优化，有效解决了LSR模型的扩展性难题，是提升大规模稀疏检索模型性能的有力工具。
---
