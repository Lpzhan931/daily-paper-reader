...
## 摘要
多适配器服务系统通常将整个序列路由到单个适配器，这在请求跨越多个领域时迫使系统必须做出单一选择。这一假设在两种重要场景下失效：(1) 多模态生成，其中同一序列中的文本和图像 token 需要不同的适配器；(2) 混合能力请求，例如“编写代码来解这个方程”，这需要来自多个专业适配器的专业知识。我们引入了逐 token 路由（per-token routing），它根据词表结构（针对多模态模型）或学习到的门控（针对语义专业化）将单个 token 路由到适配器。逐 token 路由在证明上是帕累托最优的，对于 N 个 token，其计算量为 N，而对于具有 K 种适配器类型的逐序列路由，其计算量为 K · N。我们的核心贡献是 MoLoRA (Mixture of LoRA)，它实现了可组合的专业化：加载多个特定领域的适配器，并让学习到的路由器为每个 token 选择合适的适配器。我们证明了专业化显著优于规模化：MoLoRA 使 Qwen3-1.7B 在四个推理基准测试中超过了 Qwen3-8B，而其参数量仅为后者的 1/4.7。这在推理时实现了模块化的专业知识：独立训练专注的 LoRA，无需重新训练即可将它们组合，并通过简单地加载新适配器来添加新功能。
