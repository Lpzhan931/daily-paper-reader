...
tags: ["query:llm"]
score: 7.0
evidence: 通过混合工作流降低推理成本和执行延迟
tldr: 针对现有智能体工作流生成方法依赖预定义库且推理效率低的问题，本文提出HyEvo框架。该框架通过异构原子合成，将概率性LLM节点与确定性代码节点结合，利用多岛进化策略和“反思后生成”机制自动优化工作流拓扑与逻辑。实验证明，HyEvo在推理和编程任务上显著超越现有方法，并大幅降低了推理成本和延迟，为高效智能体工作流的自动化构建提供了新方案。
selection_source: fresh_fetch
motivation: 现有的自动化工作流生成方法受限于单一的LLM推理模式和预定义算子，导致复杂任务处理效率低下且成本高昂。
method: 提出一种结合LLM语义推理与代码确定性执行的异构合成框架，并采用多岛进化策略迭代优化工作流的拓扑与逻辑。
result: 在多个基准测试中，HyEvo性能优于现有基准，且推理成本和执行延迟分别降低了高达19倍和16倍。
conclusion: HyEvo证明了通过混合异构节点与自我进化机制，可以构建出兼具高效率与高性能的自动化智能体工作流。
---
