...
## 摘要
在朴素广播模式下，多智能体 LLM 编排产生的同步成本随智能体数量 (n)、步骤 (S) 和工件大小 (|D|) 呈 O(n x S x |D|) 比例增长——我将这一机制称为“广播诱导的三重乘法开销”。我认为这种病态现象是全状态重新广播的结构性残留，而非多智能体协作的固有属性。核心主张：LLM 多智能体系统中的同步成本爆炸可以形式化地精确映射到共享内存多处理器中的缓存一致性问题，且 MESI 协议的失效机制在极小结构修改下即可迁移至工件同步。我构建了工件一致性系统 (ACS) 并证明了令牌一致性定理：当 S > n + W(d_i) 时，延迟失效（lazy invalidation）至少能将成本降低 S/(n + W(d_i))，从而将 O(n x S x |D|) 转化为 O((n + W) x |D|)。一个经过 TLA+ 验证的协议在约 2,400 个探索状态中强制执行了单写者安全性、单调版本控制和有界陈旧性。在四种工作负载配置下的仿真结果显示，在 V=0.05 时令牌节省了 95.0% +/- 1.3%，V=0.10 时为 92.3% +/- 1.4%，V=0.25 时为 88.3% +/- 1.5%，V=0.50 时为 84.2% +/- 1.3%——每一项都超过了该定理的保守下界。在 V=0.9 时，节省比例仍维持在约 81%，这与预测的崩溃阈值相反。主要贡献包括：(1) 形式化的 MESI 到工件状态映射；(2) 作为节省下界的令牌一致性定理；(3) 具有三个已证明不变量的 TLA+ 验证协议；(4) 解决了“始终读取”异议的条件工件访问语义表征；(5) 通过薄适配层与 LangGraph、CrewAI 和 AutoGen 集成的参考 Python 实现。
