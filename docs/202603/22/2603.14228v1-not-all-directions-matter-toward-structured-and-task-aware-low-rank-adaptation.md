...
motivation: 传统LoRA由于平等对待所有更新方向且各层独立适配，容易导致语义漂移和层间结构不协调，限制了微调效果。
method: 提出StructLoRA框架，利用信息瓶颈原理过滤无关方向，并通过图协调器在训练中实现跨层的一致性优化。
result: 在LLaMA、LLaVA和ViT等模型上的广泛实验证明，StructLoRA在低秩和少样本场景下显著优于现有LoRA及其变体。
conclusion: StructLoRA通过提升信息质量和结构完整性，在保持零推理开销的同时，将参数高效微调推向了更全面的优化阶段。
---
