...
tldr: "本文提出学习自适应解码策略，通过强化学习训练轻量级适配器，使LLM在推理时能根据计算资源动态调整采样参数。该方法在序列级和标记级均可实现，有效解决了固定参数无法适应任务难度差异的问题。实验显示，在MATH基准测试中，该方法在固定预算下将准确率提升了10.2%，显著优化了推理效率与性能的权衡。"
selection_source: fresh_fetch
motivation: 传统的固定采样超参数无法应对不同提示词和解码步骤中任务难度及不确定性的巨大差异。
method: 引入轻量级解码适配器，通过强化学习在序列级和标记级动态调整采样策略。
result: "在MATH基准测试中，标记级适配器在固定预算下将Pass@1准确率提升了高达10.2%。"
conclusion: 学习自适应解码策略能有效优化LLM的推理效率，在有限计算预算内实现更高的任务准确性。
---
