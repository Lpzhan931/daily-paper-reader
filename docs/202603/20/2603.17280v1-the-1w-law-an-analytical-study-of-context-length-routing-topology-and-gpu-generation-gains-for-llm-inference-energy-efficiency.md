...
title_zh: 1/W 定律：关于上下文长度路由拓扑与 GPU 代际增益对大语言模型推理能效影响的分析性研究
authors: "Huamin Chen, Xunzhuo Liu, Yuhan Liu, Junchen Jiang, Bowei He, Xue Liu"
date: 2026-03-18
pdf: "https://arxiv.org/pdf/2603.17280v1"
tags: ["query:llm"]
score: 8.0
evidence: 上下文长度和KV缓存对推理能效影响的分析研究
tldr: 本研究提出了“1/W定律”，揭示了LLM推理中每瓦令牌数随上下文窗口翻倍而减半的规律。通过分析H100等GPU的功耗与吞吐量，发现路由拓扑（如FleetOpt两池路由）在提升能效方面比硬件升级更有效。研究表明，优化上下文长度路由可提升2.5倍能效，结合B200硬件可达4.25倍，且MoE模型因激活参数量小在能效上具有显著优势，为低能耗大模型部署提供了新思路。
selection_source: fresh_fetch
motivation: 旨在探究上下文窗口大小对LLM推理能效的剧烈影响，并对比路由策略与硬件升级在提升能效上的差异。
method: 采用分析性研究方法，结合H100实测功耗、逻辑功耗模型及屋顶线模型，在inference-fleet-sim框架下进行模拟分析。
result: 推导出1/W定律，并证明FleetOpt路由策略带来的2.5倍能效提升优于从H100升级至B200的1.7倍增益。
conclusion: 上下文长度路由拓扑和MoE架构是比单纯硬件升级更强大的能效杠杆，对优化大规模推理集群的能源效率至关重要。
---

## 摘要
一个 GPU 推理集群每瓦特能交付多少 token？在相同硬件的部署中，这一数值的差异可达 40 倍——这并非源于软件效率低下，而是由于服务上下文窗口的大小。我们推导出了 1/W 定律：上下文窗口每增加一倍，每瓦特 token 数就会减半。较大的上下文窗口会降低 KV 缓存的并发限制，而 GPU 功耗基本保持不变。在 64K 上下文下，一块 H100 可同时处理 16 条序列（tok/W = 1.5）；而在 4K 上下文下，同一块 H100 可处理 256 条序列（tok/W = 17.6）。路由拓扑（决定了每块 GPU 服务的有效上下文窗口）是比购买新硬件更强大的能源杠杆。基于已发布的 H100 功耗测量数据、校准后的逻辑功耗模型以及 roofline 吞吐量模型，我们利用 inference-fleet-sim 框架通过分析推导出了这些结果；未进行新的硬件实验。与同构集群相比，双池上下文长度路由（FleetOpt）可提供约 2.5 倍的 tok/W 提升，而从 H100 升级到 B200 仅提供约 1.7 倍的提升。这些增益是相互独立的：将 FleetOpt 与 B200 结合使用，相比 H100 同构基准可实现 4.25 倍的提升。B200/H200 的数据为分析预测（存在 ±20% 的不确定性）；H100 的结果则根据已发布的测量数据进行了校准。对于 MoE 模型，激活参数权重流式传输增加了第三个杠杆。Qwen3-235B-A22B（22B 激活参数）在 H100 上的 8K 上下文下达到了约 37.8 tok/W——比 Llama-3.1-70B 高出 5.1 倍——因为解码时间随激活权重而非总参数量缩放。MoE 调度开销未计算在内，因此这是一个上限。
