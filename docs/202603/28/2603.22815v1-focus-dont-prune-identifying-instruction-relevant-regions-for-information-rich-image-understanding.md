...
tldr: 针对大型视觉语言模型在处理信息密集型图像（如文档、信息图）时因视觉标记过多导致计算开销巨大的挑战，本文提出 PinPoint 框架。该框架采用两阶段策略，首先通过指令-区域对齐技术精准定位与任务相关的关键图像区域，随后进行局部精细化特征提取。通过引入新的监督标注并优化推理流程，PinPoint 在显著降低计算成本的同时，在多个复杂视觉问答基准测试中实现了性能的全面提升。
selection_source: fresh_fetch
motivation: 大型视觉语言模型在处理复杂图像时会生成海量视觉标记，导致计算资源浪费且难以聚焦关键信息。
method: 提出 PinPoint 两阶段框架，通过指令-区域对齐定位相关区域并进行精细化特征提取，以减少冗余标记。
result: 在 InfographicVQA 和 DocVQA 等多个挑战性基准上，该方法在提高准确率的同时显著降低了计算开销。
conclusion: 研究证明，通过识别并聚焦于指令相关的图像区域，可以更高效、精准地实现对信息丰富图像的深度理解。
---
