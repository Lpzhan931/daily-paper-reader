...
motivation: 现有的视觉标记压缩方法通常依赖固定的启发式策略，难以在不同场景下自适应地平衡计算效率与信息保留。
method: 提出一种混合投影器架构，利用查询引导路由动态协调池化、重采样和剪枝三个分支，并采用类MoE机制融合输出。
result: QMoP在显著节省内存、计算量和推理时间的前提下，在多个视觉语言任务中表现优于强基线模型。
conclusion: 通过结合多种压缩策略与动态路由机制，QMoP实现了高效且灵活的视觉标记压缩，有效缓解了多模态模型的计算压力。
---
