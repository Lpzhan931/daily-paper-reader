...
tldr: 针对面向任务的目标检测（TOOD）在边缘端部署时面临的计算密集和高能耗挑战，本文提出TorR，一种受大脑启发的算法-架构协同设计方案。TorR利用超维计算（HDC）关联推理器取代CLIP式的密集对齐，并通过查询缓存、delta更新和负载门控等机制实现部分相似性复用。实验表明，TorR在保持竞争力的检测精度的同时，显著降低了能耗并实现了实时处理。
selection_source: fresh_fetch
motivation: 现有的基于CLIP的面向任务目标检测方法因计算量大和内存开销高，难以在功耗受限的边缘设备上实现实时部署。
method: 提出TorR协同设计，在算法上利用HDC重构对齐并引入缓存复用机制，在架构上实现可扩展的位切片存储器与轻量级控制器。
result: "在TSMC 28nm工艺下，TorR实现了60 FPS下的毫焦级低能耗，并在多个任务提示下达到了44.27%的平均精度。"
conclusion: 该研究证明了通过类脑HDC推理与软硬件协同优化，可以在极低功耗下实现高效、可配置的边缘端面向任务推理。
---
