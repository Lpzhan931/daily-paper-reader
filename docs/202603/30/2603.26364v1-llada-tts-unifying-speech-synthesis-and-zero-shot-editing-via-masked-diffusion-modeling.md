...
tldr: 本研究提出LLaDA-TTS，旨在解决自回归（AR）语音合成系统推理速度慢的问题。通过将预训练的AR模型转换为掩码扩散模型，并引入双向注意力机制，该方法在仅需50小时微调的情况下，实现了推理延迟与序列长度的解耦。LLaDA-TTS不仅在生成质量上媲美基准模型，且推理速度提升2倍，并原生支持零样本语音编辑任务，包括词级插入、删除和替换。
selection_source: fresh_fetch
motivation: 传统的自回归语音合成系统推理速度随序列长度增加而变慢，且缺乏灵活的语音编辑能力。
method: 利用双向注意力机制将预训练的自回归模型转换为掩码扩散模型，实现固定步数的并行生成。
result: 在Seed-TTS-Eval上达到了与基准相当的准确率，推理速度提升2倍，并实现了无需额外训练的零样本语音编辑。
conclusion: 该方法证明了AR预训练权重在声学特征局部性原理下可高效迁移至双向掩码预测，为提升TTS效率和功能性提供了通用方案。
---

## 摘要
基于大语言模型（LLM）的文本转语音（TTS）系统通过自回归（AR）解码实现了卓越的自然度，但生成 N 个语音 token 需要 N 个顺序步骤。我们提出了 LLaDA-TTS，它将 AR LLM 替换为掩码扩散模型，在固定的并行步骤内完成生成，从而使推理延迟与序列长度解耦。值得注意的是，仅使用 50 小时的微调数据，我们就通过双向注意力机制成功地将预训练的 AR 检查点迁移到了掩码扩散范式。在 64 步时，LLaDA-TTS 在 Seed-TTS-Eval 上实现了 0.98% 的字符错误率（CER，中文）和 1.96% 的词错误率（WER，英文），在达到原始 CosyVoice 3 基线性能的同时，实现了 2 倍的 LLM 阶段加速——尽管没有使用 AR 基线严重依赖的 KV 缓存优化，这一加速效果依然显著。除了加速之外，双向架构自然地支持零样本语音编辑（包括词级插入、删除和替换），且无需任何额外训练。在理论上，我们证明了在声学 token 的局部性属性下，AR 预训练权重对于双向掩码预测是近乎最优的，这解释了其快速收敛的原因。这种通用方法仅修改了注意力掩码和目标函数，可无缝应用于任何基于 LLM 的 AR TTS 系统。代码和音频样本将在 https://deft-piroshki-b652b5.netlify.app/ 提供。
