...
tldr: 针对大语言模型长文本推理中KV缓存带来的计算与带宽瓶颈，本文提出了Salca硬件加速器。通过软硬件协同设计，软件层面采用双压缩动态稀疏注意力机制及近似Top-K选择，将复杂度降至O(n)；硬件层面优化流水线架构与访存。实验表明，Salca在长文本推理上显著优于A100和现有SOTA加速器，实现了极高的吞吐量和能效比。
selection_source: fresh_fetch
motivation: 现有硬件加速器在处理长文本大模型推理时，因KV缓存导致的计算和内存带宽压力而面临严重的性能退化。
method: 采用软硬件协同设计，结合超低精度量化与特征稀疏性的双压缩动态稀疏注意力，并设计了O(n)复杂度的近似Top-K选择及全流水线并行硬件架构。
result: 相比A100 GPU，Salca实现了3.82倍的加速和74.19倍的能效提升，且吞吐量至少是现有SOTA加速器的3.5倍。
conclusion: Salca是首个能高效支持长文本推理的ASIC加速器，通过稀疏感知设计成功解决了长序列注意力机制的性能瓶颈。
---

## 摘要
长上下文提升了大语言模型的能力，但也带来了严峻的硬件挑战：计算和内存占用随序列长度线性增长。特别是在解码阶段，需要持续访问海量的 KV 缓存，极大地增加了带宽和计算压力。现有的加速器主要针对短上下文进行设计和评估，在处理长上下文时会遭遇显著的性能下降。为了弥补这一差距，我们识别了主要瓶颈，并通过软硬件协同设计提出了一种用于长上下文注意力解码的硬件加速器。在软件方面，我们提出了双重压缩动态稀疏注意力，它结合了超低精度量化与特征稀疏性，以最小化预测开销。一种硬件友好的近似 Top-K 选择进一步将过滤复杂度从 $O(n \log k)$ 降低到 $O(n)$。在硬件方面，我们深度优化了计算和内存访问，以解决稀疏注意力与长上下文之间复杂相互作用带来的瓶颈，并建立了一个性能模型来推导最优的协同设计方案。所得硬件采用全流水线并行架构，即使对于长序列也能实现 $O(n)$ 的效率。实验表明，与 A100 相比，我们的设计实现了 3.82 倍的加速和 74.19 倍的能效提升。与最先进的（SOTA）加速器相比，这是首个高效支持长上下文推理的 ASIC 加速器，吞吐量至少提高 3.5 倍，能效提高 2.08 倍。
