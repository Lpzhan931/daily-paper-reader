...
tldr: 针对边缘设备上多智能体LLM系统上下文传递效率低的问题，本文提出QKVShare框架。该框架通过量化KV缓存实现智能体间的上下文移交，结合了令牌级混合精度分配、CacheCard表示和HuggingFace兼容的注入路径。实验表明，QKVShare在保持模型性能的同时，显著降低了首字生成延迟（TTFT），尤其在大上下文场景下优势明显，为端侧多智能体协作提供了高效的系统方案。
selection_source: fresh_fetch
motivation: 边缘设备上的多智能体系统在传递上下文时面临重新预填充成本高或全精度KV传输开销大的挑战。
method: 提出QKVShare框架，利用令牌级混合精度量化、CacheCard表示和兼容HuggingFace的注入路径来实现高效的KV缓存移交。
result: 在Llama-3.1-8B上，QKVShare显著降低了TTFT（如8K上下文下从1029.7ms降至397.1ms），且自适应量化在多跳场景下优于均匀量化。
conclusion: 量化KV缓存移交是端侧多智能体系统的一个极具前景的方向，未来需进一步优化控制器消融和运行时对比。
---

## 摘要
边缘设备上的多智能体大语言模型（LLM）系统需要高效地移交潜在上下文，但目前的实际选择通常是昂贵的重新预填充（re-prefill）或全精度 KV 传输。我们研究了 QKVShare，这是一个用于智能体之间量化 KV 缓存移交的框架，它结合了令牌级混合精度分配、自包含的 CacheCard 表示以及与 HuggingFace 兼容的缓存注入路径。我们目前的结果支持一个比初稿更具体但更清晰的结论：在 Llama-3.1-8B-Instruct 上的 150 个 GSM8K 问题中，自适应量化在重复移交下保持了竞争力，并在更深跳数、更高预算的设置中，相较于均匀量化表现出最明显的优势；在移交延迟方面，QKVShare 路径在所有测试的上下文中都降低了相对于完全重新预填充的首字延迟（TTFT），从标称 1K 上下文下的 130.7 ms 对比 150.2 ms，到标称 8K 上下文下的 397.1 ms 对比 1029.7 ms。阶段计时显示，注入后的生成过程而非卡片创建过程主导了当前 QKVShare 的延迟路径。这些结果将量化 KV 移交定位为一个极具前景的端侧系统方向，同时也强调了对更强的控制器消融实验和公平运行时比较的需求。
