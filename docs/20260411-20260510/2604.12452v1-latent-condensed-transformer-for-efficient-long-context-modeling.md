...
tldr: 针对大语言模型长文本处理中KV缓存增长和计算复杂度高的问题，本文提出Latent-Condensed Attention (LCA)。LCA在MLA的潜空间内直接压缩上下文，通过查询感知池化聚合语义向量并保留位置锚点，实现了计算开销和KV缓存的同步降低。该方法不增加参数且具有架构通用性，在保持性能的同时显著提升了长文本处理效率。
selection_source: fresh_fetch
motivation: 现有的KV缓存压缩技术与稀疏注意力方法无法在原生结构上协同工作，限制了长文本建模的进一步优化。
method: 提出LCA机制，在潜空间内将表示解耦为语义向量和位置键，分别采用查询感知池化和锚点选择进行上下文压缩。
result: "在128K上下文下，该方法实现了2.5倍的预填充加速和90%的KV缓存缩减，并具有长度无关的理论误差界限。"
conclusion: LCA通过在潜空间进行联合优化，为大语言模型提供了一种高效、低开销且通用的长文本处理方案。
---

## 摘要
大语言模型 (LLM) 在处理长上下文时面临重大挑战，原因在于键值 (KV) 缓存的线性增长以及自注意力机制的二次复杂度。现有方法分别解决这些瓶颈：多头潜注意力 (MLA) 通过将 token 投影到低维潜空间来减少 KV 缓存，而稀疏注意力则减少了计算量。然而，稀疏方法无法原生运行在 MLA 的压缩潜空间结构上，从而错失了联合优化的机会。在本文中，我们提出了潜空间压缩注意力 (LCA)，它直接在 MLA 的潜空间内压缩上下文，其中表示被解耦为语义潜向量和位置键。LCA 通过查询感知池化分别聚合语义向量，并通过锚点选择保留位置键。该方法在不增加参数的情况下，同时降低了计算成本和 KV 缓存。除了 MLA，LCA 的设计与架构无关，可以轻松扩展到其他注意力机制（如 GQA）。在理论上，我们证明了一个与长度无关的误差界限。实验表明，在 128K 上下文下，LCA 实现了高达 2.5 倍的预填充加速和 90% 的 KV 缓存减少，同时保持了具有竞争力的性能。
