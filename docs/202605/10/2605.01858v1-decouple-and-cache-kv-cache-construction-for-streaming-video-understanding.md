...
tldr: "针对流式视频理解中内存受限和长序列泛化难的问题，本文提出了一种无需训练的缓存构建机制 DSCache。该方法通过解耦累积的历史缓存与即时的当前缓存，并结合位置无关的编码策略，使预训练离线模型能有效处理无限视频流。实验证明，DSCache 在流式视频问答任务中取得了 SOTA 性能，平均准确率提升了 2.5%。"
selection_source: fresh_fetch
motivation: 现有的流式视频大模型难以扩展到无限视频流，且在从短序列训练向长流推理泛化时存在缓存构建效率和位置溢出的挑战。
method: 提出 DSCache 机制，通过解耦历史累积缓存与即时按需缓存，并引入位置无关编码策略来实现位置外推。
result: "在流式视频问答基准测试中，DSCache 相比现有方法实现了平均 2.5% 的准确率提升，展现了卓越的性能。"
conclusion: DSCache 证明了通过合理的缓存解耦和位置编码优化，无需重新训练即可显著提升离线模型在长视频流理解中的表现。
---

## 摘要
流式视频理解需要在有限的内存和计算资源下处理无限的视频流，这带来了两个关键挑战。首先，对于无限流，需要不断构建新的并剔除旧的键值（KV）缓存。其次，由于在无限流上收集数据和训练的成本很高，模型必须从短序列中学习，同时泛化到长流。现有的流式视频大语言模型（VideoVLLMs）无法扩展到无限视频流，或者侧重于缓存重用策略，而对缓存构建的影响研究不足。在本文中，我们提出了解耦流式缓存（DSCache），这是一种无需训练的缓存构建机制，可将预训练的离线模型适配到流式场景。DSCache 维护一个累积的历史 KV 缓存，同时根据需要构建一个独立的即时缓存，该缓存与历史缓存解耦，以保留近期输入的丰富信息。为了实现超出训练长度的位置外推，DSCache 进一步结合了位置无关的编码策略，确保 KV 缓存支持未见过的位置并防止位置溢出。在流式视频问答（Streaming Video QA）基准测试上的实验表明，DSCache 达到了最先进的性能，比之前的方法平均准确率提高了 2.5%。
