...
## 摘要
在黑盒大语言模型（LLM）服务中，响应的可靠性在决策时通常仅是部分可观测的，而更强的推理路径会产生巨大的计算成本，从而引发了一个受预算限制的序列决策问题：对于每个请求，系统必须决定默认的低成本响应是否足够可靠，或者是否应分配额外的计算资源以提高响应质量。在本文中，我们提出了面向风险感知推理控制的可验证观测（Veroic），这是一个用于黑盒 LLM 环境下自适应推理控制的框架。该框架将请求时的控制建模为部分可观测马尔可夫决策过程（POMDP），以捕捉部分可观测性和序列预算耦合。它通过将异构质量信号聚合为潜在响应可靠性的信念状态，从输入-输出对中构建了一个轻量级的可验证观测通道，随后预算感知策略利用该状态来决定是返回默认输出还是触发更高成本的推理路径。在多种任务上的实验表明，与竞争基准相比，Veroic 实现了更好的质量-成本权衡、更强的风险估计与校准，以及更稳健的长时程推理控制。
