...
## 摘要
长上下文大语言模型（LLM）推理的瓶颈不在于计算，而在于每个解码步骤中扫描 KV 缓存所带来的 O(n) 内存带宽开销——这是一道任何算力扩展都无法打破的墙。近期的光子加速器在密集注意力计算方面展示了令人印象深刻的吞吐量；然而，当应用于长上下文时，这些方法继承了与电子注意力相同的 O(n) 内存缩放特性。我们观察到，真正的突破点在于粗粒度的块选择步骤：这是一种决定获取哪些 KV 块的受内存限制的相似性搜索。我们首次发现，该任务在结构上与光子“广播与加权”（broadcast-and-weight）范式相匹配——查询通过无源分束扇出到所有候选块，特征签名是准静态的（匹配电光微环谐振器 MRR 编程），且仅排序顺序重要（可将精度放宽至 4-6 位）。至关重要的是，光子优势随上下文长度增加而增长：随着 N 的增加，电子扫描成本线性上升，而光子评估保持在 O(1)。我们在 PRISM（通过微环权重的内积相似度进行光子排序）中实例化了这一见解，这是一种薄膜铌酸锂（TFLN）相似性引擎。在 Qwen2.5-7B 上进行的受硬件损伤影响的“大海捞针”评估证实，在 k=32 时，从 4K 到 64K token 均能保持 100% 的准确率，且在 64K 上下文下流量减少了 16 倍。在实际上下文长度（n >= 4K）下，PRISM 相比 GPU 基准实现了四个数量级的能效优势。
