...
tags: ["query:llm"]
score: 8.0
evidence: 用于高效多模态大模型推理的KV缓存兼容剪枝
tldr: "针对大型视觉语言模型（LVLM）处理高分辨率图像时计算开销巨大的问题，本文提出ASAP，一种无需训练且兼容KV缓存的剪枝方案。ASAP通过动态双向软注意力掩码缓解“注意力偏移”现象，并利用加权软合并减少语义冗余。该方法在保持LLaVA-NeXT-7B约99%性能的同时，大幅减少了约80%的计算量，实现了高效的视觉上下文压缩。"
selection_source: fresh_fetch
motivation: 现有的视觉Token减少策略忽略了LVLM中导致Token评分偏差的“注意力偏移”现象以及Token间的语义冗余。
method: 提出ASAP方案，结合动态双向软注意力掩码以精准识别关键Token，并采用加权软合并技术融合语义相似的视觉块。
result: "在LLaVA-NeXT-7B上实现了近乎无损的压缩，保留了99.02%的性能，同时将计算量（FLOPs）降低了约80%。"
conclusion: ASAP为LVLM提供了一种高效、无需训练且兼容现有推理架构的视觉Token剪枝新范式。
---

## 摘要
尽管大型视觉语言模型（LVLMs）展现出卓越的多模态能力，但处理高分辨率视觉 Token 的二次方计算成本仍然是一个关键瓶颈。尽管最近的 Token 减少策略试图加速推理，但这些方法未能充分利用注意力值，且无法解决 Token 冗余问题。更关键的是，它们忽视了 LVLMs 中固有的“注意力偏移”现象，该现象会导致 Token 注意力分数产生偏差。在这项工作中，我们提出了 ASAP，这是一种新型的无需训练且兼容 KV-Cache 的剪枝方案，全面解决了上述局限性。首先，我们通过利用动态双向软注意力掩码来缓解注意力偏移，确保选择真正具有信息量的 Token，而非基于朴素注意力的选择。其次，我们认为 Token 集合中的高语义冗余会降低性能。因此，我们引入了一个加权软合并组件，用于合并语义相似的 Token，仅为后续层保留特征最密集的视觉补丁。ASAP 实现了视觉上下文的几乎无损压缩，在保留 LLaVA-NeXT-7B 原始性能 99.02% 的同时，大幅削减了约 80% 的计算 FLOPs。
