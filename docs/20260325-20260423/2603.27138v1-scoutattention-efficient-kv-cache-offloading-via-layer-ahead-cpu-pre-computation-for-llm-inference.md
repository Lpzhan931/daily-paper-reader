...
tldr: 针对大语言模型长文本推理中KV缓存导致的GPU显存瓶颈，本文提出ScoutAttention框架。该框架通过GPU-CPU协同计算，引入块稀疏注意力机制以减轻CPU负担，并创新性地采用“提前一层”的CPU预计算算法和异步周期性召回机制。实验表明，该方法在保持高精度的同时，推理速度比现有卸载方案提升了2.1倍，有效缓解了I/O和计算瓶颈。
selection_source: fresh_fetch
motivation: 现有的KV缓存卸载技术受限于频繁的数据传输或沉重的CPU计算负担，导致GPU利用率低下且推理延迟增加。
method: 提出GPU-CPU协同的块稀疏注意力机制，并结合“提前一层”的CPU预计算与异步周期性召回算法来优化计算流程。
result: "在保持精度损失低于2.4%的前提下，ScoutAttention相比现有卸载方法实现了2.1倍的推理加速。"
conclusion: ScoutAttention通过优化CPU计算时机与稀疏化策略，有效解决了长文本推理中的KV缓存卸载性能瓶颈。
---
