...
## 摘要
多模态大语言模型 (MLLM) 近期在理解多样化视觉输入（包括高分辨率图像和长视频序列）并生成响应方面展现出了强大的能力。随着这些模型向更丰富的视觉表示扩展，推理过程日益依赖于在键值 (KV) 缓存中存储大量的视觉 token，使得显存消耗成为核心瓶颈。现有方法通过识别视觉 token 中的冗余并压缩缓存来解决这一问题，但此类压缩通常仅在处理完所有输入后才进行，导致预填充阶段的峰值显存占用过高。在这项工作中，我们展示了 MLLM 表现出固有的结构规律性和表示冗余，可被利用来控制整个推理过程中的显存增长。基于这一见解，我们提出了一种顺序输入压缩机制，通过在预填充过程中执行结构感知的键值缓存压缩，从而强制执行固定的显存预算。该方法在大幅降低峰值显存占用的同时，仅以极小的性能损失维持了生成表现，实现了更具实用性且显存高效的多模态推理。
