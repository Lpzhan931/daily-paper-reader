...
tags: ["query:llm"]
score: 6.0
evidence: 基于查找表的DRAM-PIM量化神经网络推理
tldr: 本研究针对DRAM-PIM架构逻辑密度受限但内存带宽丰富的特点，提出LOCALUT框架。该框架利用查找表（LUT）替代传统算术逻辑，通过将多个MAC操作打包进单个LUT来平衡容量与计算。核心贡献包括：通过LUT规范化消除冗余、引入重排序LUT简化权重映射，以及采用LUT切片流式传输优化内存层级利用。在UPMEM真实系统上，该方案实现了1.82倍的几何平均加速，为低功耗、可扩展的PIM推理提供了新思路。
selection_source: fresh_fetch
motivation: DRAM-PIM架构逻辑资源有限但内存充足，传统算术运算效率低，而LUT能通过消耗内存容量来换取更高的计算吞吐量。
method: 提出LOCALUT，通过操作打包LUT、规范化去冗余、轻量级重排序LUT以及切片流式传输策略，优化低比特量化DNN的推理效率。
result: 在UPMEM真实硬件系统上评估显示，针对多种数值精度和DNN模型，LOCALUT实现了1.82倍的几何平均加速。
conclusion: LOCALUT证明了在PIM设计中利用容量-计算权衡的有效性，为基于LUT的高效、低逻辑开销DNN推理开辟了新路径。
---
