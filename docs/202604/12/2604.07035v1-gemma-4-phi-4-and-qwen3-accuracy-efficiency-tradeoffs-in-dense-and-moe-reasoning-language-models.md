...
tags: ["query:llm"]
score: 7.0
evidence: MoE与稠密模型的准确率-效率权衡
tldr: 本研究针对Gemma-4、Phi-4和Qwen3等最新推理模型，深入探讨了稠密与混合专家（MoE）架构在实际推理约束下的准确率与效率权衡。通过对7款模型在ARC、GSM8K等四个基准测试及三种提示策略下的8400次评估，记录了准确率、显存占用和计算量等关键指标。研究发现，Gemma-4-E4B在综合表现上最优，而MoE架构并非总能提供最佳的资源利用效率，提示策略对结果影响显著。
selection_source: fresh_fetch
motivation: 探究在实际推理约束下，混合专家（MoE）模型是否真的比稠密模型在准确率与效率的权衡上更具优势。
method: 对Gemma-4、Phi-4和Qwen3等7款推理导向模型在四个基准测试上进行8400次评估，记录准确率、延迟、显存占用及FLOPs。
result: Gemma-4-E4B在少样本思维链下表现最优，而MoE架构的Gemma-4-26B-A4B虽准确率接近但显存消耗显著更高。
conclusion: 稀疏激活并不保证最佳实际运行点，准确率与效率的权衡取决于架构、提示策略和任务构成的共同作用。
---

## 摘要
混合专家（MoE）语言模型通常被认为比稠密模型具有更好的质量-效率权衡，因为每个 token 仅激活一部分参数，但这种优势的实际价值取决于现实推理约束下的端到端表现。我们对七个近期面向推理的指令微调模型进行了受控实证基准测试，涵盖了稠密和 MoE 设计，具体包括 Gemma-4-E2B、Gemma-4-E4B、Gemma-4-26B-A4B、Phi-4-mini-reasoning、Phi-4-reasoning、Qwen3-8B 和 Qwen3-30B-A3B。评估在四个基准测试（ARC-Challenge、GSM8K、Math Level 1-3 和 TruthfulQA MC1）上进行，采用了三种提示策略：零样本（zero-shot）、思维链（chain-of-thought）以及少样本思维链（few-shot chain-of-thought）。该研究共涵盖 8,400 次模型-数据集-提示评估，并记录了准确率、延迟、峰值 GPU 显存占用（VRAM）以及近似的每 token 浮点运算量（FLOPs）指标。在加权多任务汇总中，采用少样本思维链的 Gemma-4-E4B 取得了最佳综合结果，加权准确率达到 0.675，平均显存占用为 14.9 GB；而 Gemma-4-26B-A4B 的准确率接近（0.663），但显存占用显著更高，达到 48.1 GB。在任务层面，Gemma 模型在 ARC 和 Math 上占据主导地位，Phi 模型在 TruthfulQA 上表现最强，而 GSM8K 显示出最大的提示敏感性，其中 Phi-4-reasoning 的准确率从思维链下的 0.67 骤降至少样本思维链下的 0.11。这些结果表明，仅靠稀疏激活并不能保证最佳的实际运行点：观察到的准确率-效率权衡共同取决于架构、提示协议和任务构成。我们发布了一个可重复的基准测试流水线、汇总结果以及配对统计分析，以支持在真实资源约束下对推理大语言模型（LLM）进行面向部署的评估。
