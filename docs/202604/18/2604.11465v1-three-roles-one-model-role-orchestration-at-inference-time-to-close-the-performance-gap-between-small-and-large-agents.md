...
## 摘要
大语言模型（LLM）智能体在现实的工具使用任务中展现出巨大潜力，但在中低端硬件上部署高性能智能体仍具挑战。我们研究了在不增加任何额外训练计算量的情况下，仅通过推理时脚手架（inference-time scaffolding）能否提升小型模型在复杂多步环境中的性能。在单张 24 GB GPU 上，我们在全精度（FP16，12K 上下文）和 4 位量化（AWQ，32K 上下文）配置下评估了 Qwen3-8B。在没有任何干预的情况下，原始模型的任务目标完成率仅为 5.4% (FP16) 和 3.0% (AWQ)。在系统性故障模式分析的指导下，我们引入了一个三层推理脚手架流水线，将同一个冻结模型部署为三种不同的角色：(1) 摘要模型，在压缩对话历史的同时保留关键工件（如 token、凭据、API 响应）；(2) 主智能体模型，在压缩后的上下文基础上进行推理；(3) 独立修正模型，在不访问对话历史的情况下审查并修改智能体的代码输出，从而打破重复的失败循环。应用于同一个未经修改的模型时，该脚手架实现了 8.9% (FP16) 和 5.9% (AWQ) 的任务目标完成率，在两种设置下性能几乎翻倍，尤其在难度 1 的任务上提升显著（FP16 从 15.8% 提升至 26.3%；AWQ 从 5.3% 提升至 14.0%）。在全精度推理中，我们带有脚手架的 8B 模型超过了原始 AppWorld 评估中的 DeepSeek-Coder 33B Instruct (7.1%)，证明了结构化的推理时干预可以使小型模型具备与 4 倍于其规模的系统竞争的能力。我们将该方法形式化为基于冻结基座模型的脚手架策略，即通过不同的条件约束对同一权重进行三次调用，并将其与强化学习中的测试时计算扩展（test-time compute scaling）和动作空间塑造（action-space shaping）联系起来。
