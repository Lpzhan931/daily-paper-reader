...
tldr: 该研究针对流式视频生成中滑动窗口注意力导致的历史信息丢失和计算开销大等问题，提出了Hybrid Forcing框架。通过引入轻量级线性时间注意力和块稀疏注意力，结合解耦蒸馏策略，实现了长程依赖的保留与计算效率的提升。该方法在保持SOTA性能的同时，在单张H100显卡上实现了832x480分辨率下29.5 FPS的实时、无限长视频生成。
selection_source: fresh_fetch
motivation: 现有的流式视频生成方法在长视频生成中容易丢失远期历史信息，且滑动窗口注意力的计算开销难以满足实时部署需求。
method: 提出混合注意力机制，利用线性时间注意力保留长程上下文并结合块稀疏注意力优化局部计算，同时采用解耦蒸馏策略确保模型稳定优化。
result: 在多个基准测试中达到领先水平，并能在单卡H100上以29.5 FPS的速度生成实时、无限制长度的高清视频。
conclusion: Hybrid Forcing通过创新的混合注意力和蒸馏方案，有效解决了流式视频生成的效率与长程一致性难题，为实时长视频生成提供了高效方案。
---
