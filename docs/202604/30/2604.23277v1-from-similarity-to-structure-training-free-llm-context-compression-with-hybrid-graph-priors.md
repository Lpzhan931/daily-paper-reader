...
tags: ["query:llm"]
score: 8.0
evidence: 用于高效长文本处理的免训练LLM上下文压缩
tldr: 针对长上下文大模型计算开销大且处理超长输入不可靠的问题，本文提出一种无需训练且模型无关的上下文压缩框架。该方法利用混合图先验（结合语义与序列关系）构建句子图，通过聚类提取主题骨架，并结合任务相关性、代表性及中心度等指标对句子排序，最后通过贪婪选择生成兼顾覆盖度与连贯性的压缩文本，在长文档任务中表现出色。
selection_source: fresh_fetch
motivation: 现有压缩方法在严格 Token 预算下难以平衡任务相关性、主题覆盖度及跨句连贯性。
method: 构建结合语义 k-NN 与序列边的混合句子图，通过聚类提取主题骨架并利用多维可解释评分进行贪婪句子选择。
result: 在四个数据集上的实验证明，该方法在长文档基准测试中优于现有的抽取式和生成式基准模型。
conclusion: 引入结构化图先验为无需训练的上下文压缩提供了有效路径，显著提升了长文本处理的效率与质量。
---

## 摘要
长上下文大语言模型（LLM）的运行计算成本依然高昂，且往往无法可靠地处理极长输入，这使得上下文压缩成为许多系统的重要组成部分。现有的压缩方法通常依赖于经过训练的压缩器、稠密检索式选择或启发式裁剪，在严格的 token 预算下，它们往往难以同时兼顾任务相关性、主题覆盖范围和跨句连贯性。为解决这一问题，我们提出了一种免训练且与模型无关的压缩框架，该框架在结构化图先验的指导下选择紧凑的句子集。我们的方法构建了一个稀疏混合句子图，结合了互 k-NN 语义边和短程序列边，通过聚类提取主题骨架，并使用集成任务相关性、聚类代表性、桥接中心性和环覆盖线索的可解释评分对句子进行排序。随后，采用带有冗余抑制的有预算贪婪选择，按原始顺序生成可读的压缩上下文。在四个数据集上的实验结果表明，我们的方法与强大的抽取式和生成式基准模型相比具有竞争力，在长文档基准测试中表现出更大的增益。
