...
## 摘要
某些文本生成任务（如属性值提取，AVE）需要从同一文档上下文中解码多个独立的序列。虽然标准的自回归解码因其顺序特性而速度较慢，但输出序列之间的独立性为并行化提供了机会。我们提出了超并行解码（Hyper-Parallel Decoding, HPD），这是一种新型解码算法，通过利用跨批次的共享内存和计算来加速离线解码。HPD 通过位置 ID 操作实现乱序 Token 生成，显著提高了效率。在 AVE 上的实验表明，属性-值对是条件独立的，这使我们能够在每个提示词（prompt）内并行化生成值。通过在单个提示词中进一步堆叠多个文档，我们可以在每个提示词中并行解码多达 96 个 Token。HPD 适用于所有大语言模型（LLM），在不损失输出质量的前提下，可将推理成本和总推理时间降低多达 13.8 倍，有望在工业级 AVE 任务中节省数十万美元。尽管 HPD 是为属性提取设计的，但它没有针对 AVE 领域的特殊假设，理论上可以应用于其他具有独立输出结构的场景。
