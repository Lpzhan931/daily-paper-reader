...
tags: ["query:llm"]
score: 6.0
evidence: LLM服务基础设施的拓扑优化
tldr: 针对大模型数据中心中光电路交换（OCS）静态拓扑优化问题，本文提出DELTA框架。该框架摒弃了传统的流量矩阵模型，转而利用计算-通信有向无环图（DAG）捕捉时变流量模式，并将其建模为混合整数线性规划（MILP）。通过利用非关键任务的时间裕量节省光端口，并采用变长时段和双轨加速策略提升求解效率。实验表明，DELTA在减少通信延迟和节省端口资源方面显著优于现有方案，实现了近乎理想的网络性能。
selection_source: fresh_fetch
motivation: 传统的流量矩阵模型无法捕捉并行策略下的突发并发带宽需求，且忽略了并发通信所需的独立通道，导致OCS拓扑优化效果受限。
method: 提出基于DAG感知的MILP模型，利用变长时段减少解空间，并通过搜索空间剪枝和启发式热启动实现大规模集群的快速求解。
result: "相比现有技术，DELTA减少了高达17.5%的通信时间，节省了至少20%的光端口，并将与理想无阻塞网络的性能差距缩小了26.1%。"
conclusion: DELTA证明了通过深度结合任务调度DAG与物理拓扑优化，可以有效提升OCS在AI工作负载下的资源效率和通信性能。
---

## 摘要
大语言模型（LLM）规模的快速增长加剧了 AI 数据中心（AIDC）的通信瓶颈。为解决这一问题，光电路交换（OCS）因其卓越的带宽容量和能效而得到越来越多的应用。然而，其重构开销使得迭代内拓扑更新变得不可行，因此需要预先设计静态拓扑以吸收随时间变化的 LLM 流量。现有方法通常基于流量矩阵设计这些拓扑。然而，这种表示方式掩盖了由并行化策略决定的突发并发带宽需求，且未能考虑到并发通信所需的独立通道。为此，我们提出了 DELTA，这是一个高效的 AIDC 逻辑拓扑优化框架。它利用计算-通信有向无环图（DAG）将随时间变化的流量模式编码为混合整数线性规划（MILP）模型，同时利用非关键任务的时间松弛（temporal slack）来节省光端口，且不损害迭代完成时间（makespan）。通过开创性的变长时间间隔建模，DELTA 相比于固定步长建模显著缩小了求解空间。为了扩展到千卡 GPU 集群，我们设计了一种双轨加速策略，结合了搜索空间剪枝（将复杂度从平方级降低到线性级）和启发式热启动。对大规模 LLM 工作负载的评估表明，与最先进的基于流量矩阵的基准方法相比，DELTA 将通信时间缩短了高达 17.5%。此外，该框架将光端口消耗降低了至少 20%；将这些剩余端口动态重新分配给带宽受限的工作负载，可将其相对于理想无阻塞电网络的性能差距缩小高达 26.1%，最终使大多数工作负载能够达到近乎理想的性能。
