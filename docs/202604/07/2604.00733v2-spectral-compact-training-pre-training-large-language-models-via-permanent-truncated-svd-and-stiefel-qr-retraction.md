...
motivation: 内存墙是限制在消费级硬件上训练大规模语言模型的主要瓶颈。
method: 采用永久截断SVD因子替代稠密权重矩阵，并通过QR分解将因子回缩至Stiefel流形以确保正交性。
result: 在rank 32下实现MLP层199倍内存缩减，使70B模型能在7.2GB内存设备上训练，且吞吐量提升一倍。
conclusion: 实验证明学习率调度而非模型秩是性能的关键瓶颈，SCT为受限硬件下的高效预训练提供了新路径。
---

## 摘要
内存墙仍然是在消费级硬件上训练大语言模型的主要瓶颈。我们引入了谱紧凑训练（SCT），这是一种用永久截断 SVD 因子 W = U diag(s) V^T 替换稠密权重矩阵的方法，在训练或推理过程中永远不会实例化完整的稠密矩阵。梯度通过标准反向传播流经紧凑的谱因子，并且在每个优化器步骤后，U 和 V 通过 QR 分解被回缩到 Stiefel 流形。在秩为 32 时，SCT 在每个 MLP 层实现了高达 199 倍的内存减少，使得在 Steam Deck 掌机上对 70B 参数架构进行完整训练步骤成为可能（峰值内存为 7.2 GB，而使用 Adam 的稠密 FP32 训练则需要 1,245 GB）。在 SmolLM2-1.7B 上进行的秩扫描实验（秩 32-256，2000 步，NVIDIA A100）表明，所有测试的秩都收敛到相同的损失底限（约 4.2-4.5），这表明学习率调度（而非 MLP 秩）是主要的瓶颈。秩 128 成为效率的最佳平衡点，在 11.7 倍的 MLP 压缩下具有最低的困惑度。在秩为 32 时，GPU 内存下降了 46%，同时训练吞吐量翻了一番。
