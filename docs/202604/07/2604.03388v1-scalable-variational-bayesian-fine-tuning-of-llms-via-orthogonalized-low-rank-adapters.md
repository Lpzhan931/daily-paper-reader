...
score: 6.0
evidence: 贝叶斯微调的高效推理
tldr: 针对大语言模型在微调中存在的过度自信和不确定性量化难题，本文提出PoLAR-VBLL框架。该方法结合了正交化低秩适配器（PoLAR）以增强表达能力，并采用变分贝叶斯最后一层（VBLL）模型。通过交替优化PoLAR参数和最后一层后验分布，实现了高效且可扩展的贝叶斯微调，在提升模型泛化性能的同时，显著改善了推理时的不确定性估计。
selection_source: fresh_fetch
motivation: 现有的参数高效微调方法在特定领域任务中容易产生过度自信，且现有的贝叶斯推理方法在可扩展性或校准效果上存在局限。
method: 提出PoLAR-VBLL框架，利用极分解正交化适配器解决秩崩溃问题，并结合变分贝叶斯最后一层模型进行联合优化。
result: 实验证明该方法在多种常识推理任务的分布内和分布外数据上，均表现出优异的泛化能力和校准后的不确定性估计。
conclusion: PoLAR-VBLL成功将架构增强的优化与可扩展的贝叶斯推理相结合，为大语言模型提供了可靠且高效的不确定性量化方案。
---

## 摘要
在将大语言模型（LLMs）部署到安全关键型应用时，不确定性量化（UQ）对于自我评估基于 LLM 决策的可靠性至关重要。然而，此类决策通常存在过度自信的问题，尤其是在针对数据有限的下游特定领域任务进行参数高效微调（PEFT）之后。现有缓解该问题的方法要么依赖于基于拉普拉斯近似的后验框架（这可能因训练轨迹的不同而导致次优的校准效果），要么依赖于变分贝叶斯训练（这在推理时需要对整个 LLM 骨干网络进行多次完整的正向传播以进行蒙特卡洛估计，从而给部署带来了可扩展性挑战）。为了解决这些局限性，我们基于贝叶斯最后一层（BLL）模型，其中基于 LLM 的确定性特征提取器后接随机的最后一层参数以进行不确定性推理。由于现有的用于 PEFT 的低秩适配器（LoRA）因秩崩溃而表现力有限，我们通过极分解低秩适配器表示（PoLAR）来解决这一问题，这是一种结合黎曼优化的正交化参数化方法，旨在实现更稳定且更具表现力的适配。基于此 PoLAR-BLL 模型，我们利用变分（V）推理框架提出了一种可扩展的贝叶斯微调方法，该方法通过交替优化共同寻求 PoLAR 参数和最后一层参数的近似后验。由此产生的 PoLAR-VBLL 是一个灵活的框架，它将架构增强优化与可扩展贝叶斯推理有机结合，赋予 LLM 经过良好校准的 UQ 能力。我们的实证结果验证了 PoLAR-VBLL 在各种常识推理任务的分布内和分布外数据上的泛化能力和不确定性估计方面的有效性。
