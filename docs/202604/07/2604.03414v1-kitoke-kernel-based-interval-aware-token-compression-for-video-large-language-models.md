...
tldr: "针对视频大语言模型（Video LLMs）推理成本高的问题，本文提出KiToke，一种无需训练且与查询无关的Token压缩方法。该方法利用基于核函数的冗余度量来全局估计Token多样性，实现内容自适应选择，并通过轻量级时间间隔构建与感知合并来保持时间连贯性。实验证明，KiToke在多个基准测试中优于现有方法，尤其在仅保留1% Token的极端压缩情况下表现出色。"
selection_source: fresh_fetch
motivation: 视频大语言模型中海量的视觉Token导致了极高的推理成本和计算负担。
method: 提出基于核函数的全局冗余度量进行Token选择，并结合时间间隔感知合并技术以维持视频的时间连贯性。
result: "在多个视频理解基准测试中，KiToke在极低保留率（低至1%）下仍能显著优于现有的无需训练的压缩方法。"
conclusion: KiToke通过显式捕捉全视频范围内的全局冗余，为视频模型提供了一种高效且鲁棒的Token压缩方案。
---
