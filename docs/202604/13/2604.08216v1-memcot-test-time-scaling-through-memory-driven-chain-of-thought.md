...
tldr: 针对大语言模型在长文本推理中存在的幻觉和遗忘问题，本文提出MemCoT框架。该框架将长文本推理转化为迭代的状态化搜索过程，通过多视图长期记忆感知模块实现证据定位与上下文扩展，并结合双重短期记忆系统记录搜索决策。MemCoT在多个长文本基准测试中达到SOTA水平，有效提升了模型的测试时推理能力。
selection_source: fresh_fetch
motivation: 现有的静态单步检索机制在处理海量碎片化长文本时，容易导致语义稀释和上下文断裂。
method: 提出MemCoT框架，利用多视图长期记忆进行证据定位与重构，并结合双重短期记忆引导动态查询分解。
result: 在LoCoMo和LongMemEval-S等基准测试中，MemCoT助力多种开源及闭源模型取得了当前最佳性能。
conclusion: MemCoT通过将推理过程重定义为记忆驱动的迭代搜索，显著增强了LLM在复杂长上下文下的因果推理能力。
---

## 摘要
大语言模型（LLMs）在处理海量、碎片化的长上下文进行因果推理时，仍面临严重的幻觉和灾难性遗忘问题。现有的记忆机制通常将检索视为一种静态、单步的被动匹配过程，导致严重的语义稀释和上下文碎片化。为了克服这些根本性瓶颈，我们提出了 MemCoT，这是一种测试时记忆扩展框架，它通过将长上下文推理转化为迭代式、有状态的信息搜索，重新定义了推理过程。MemCoT 引入了一个多视图长期记忆感知模块，支持“缩小范围（Zoom-In）”的证据定位和“扩大范围（Zoom-Out）”的上下文扩展，使模型能够首先识别相关证据所在的位置，随后重构推理所需的周边因果结构。此外，MemCoT 采用了一个由语义状态记忆和情节轨迹记忆组成的任务条件双重短期记忆系统。该短期记忆记录了历史搜索决策，并动态指导跨迭代的查询分解与剪枝。实验评估表明，MemCoT 取得了最先进的性能。在 MemCoT 的赋能下，多个开源和闭源模型在 LoCoMo 基准测试和 LongMemEval-S 基准测试中均达到了 SOTA 性能。
