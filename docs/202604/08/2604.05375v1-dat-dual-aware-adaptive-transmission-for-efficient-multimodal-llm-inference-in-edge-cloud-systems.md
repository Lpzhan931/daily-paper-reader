...
tags: ["query:llm"]
score: 8.0
evidence: 通过大小模型级联实现高效多模态大模型推理
tldr: 针对边缘云系统中多模态大模型（MLLM）处理视频流时的高延迟和高带宽开销问题，本文提出DAT框架。该框架通过边云协同的小-大模型级联过滤冗余帧，结合视觉引导的微调策略提升理解能力，并设计了语义与带宽感知的自适应传输优化方法。实验证明，DAT在保证高准确率的同时，显著降低了告警延迟并提升了视觉证据的传输效率。
selection_source: fresh_fetch
motivation: 解决多模态大模型在带宽受限的边缘云系统中处理连续视频流时，计算和通信开销过大导致的高延迟问题。
method: 提出一种边云协同的级联推理架构，结合视觉引导的微调策略以及语义与带宽感知的多流自适应传输优化。
result: "实现了98.83%的识别准确率，在严重拥塞下将语义告警延迟降低了77.5%，并确保绝大部分视觉证据在0.5秒内送达。"
