...
tags: ["query:llm"]
score: 6.0
evidence: 用于高效代码生成的推理时反馈机制
tldr: 针对大语言模型生成的代码虽然正确但效率低下的问题，本文提出了一种无需微调的推理时反馈机制 EffiPair。该方法通过“相对对比反馈”（RCF）比较两个结构相似但效率不同的程序，提取导致性能差异的关键信息。EffiPair 在测试时迭代优化，通过识别效率差距大的程序对并总结执行差异，引导模型生成更高效的代码。实验证明其在提升代码运行速度的同时，显著降低了计算开销。
selection_source: fresh_fetch
motivation: 现有的代码效率优化方法依赖绝对执行反馈（如单程序耗时），成本高且指导性弱。
method: 提出 EffiPair 框架，通过对比两个功能相同但效率不同的程序对，提取相对对比反馈（RCF）来指导代码迭代优化。
result: "在 DeepSeek-Chat V3.2 上实现了高达 1.5 倍的加速，且相比之前的方法减少了 90% 以上的 Token 消耗。"
conclusion: 相对对比反馈比孤立的标量反馈更有效，能在不更新模型参数的情况下显著提升 LLM 生成代码的运行效率。
---
