...
tldr: 针对边缘端NPU在LLM推理中受限于片上内存且eDRAM刷新功耗高的问题，本文提出SHIELD架构。该架构利用BF16激活值的生命周期和位级敏感度，将符号/指数位与尾数位分离，对瞬态QO尾数禁用刷新，对持久KV尾数放宽刷新，在保持精度的同时显著降低了能耗。
selection_source: fresh_fetch
motivation: 边缘NPU推理中eDRAM的定期刷新消耗大量能量，且现有研究忽略了瞬态激活值的容错特性。
method: 提出一种生命周期感知的分段eDRAM架构，通过分离BF16字段并针对不同类型的激活值尾数实施差异化刷新策略。
result: "在多种LLM场景下，SHIELD相比标准刷新基准降低了35%的eDRAM刷新能耗，且不影响模型精度。"
conclusion: SHIELD证明了结合数据生命周期与位级敏感度的内存管理策略能有效提升边缘端LLM推理的能效。
---
