...
motivation: 现有的代码选择方法通常依赖昂贵或随机的外部验证器，难以可靠地识别正确解。
method: 提出符号等价划分框架，利用符号执行将候选程序按语义等价性分组，并结合SMT约束优化路径搜索。
result: 在N=10时，HumanEval+的平均准确率从0.728提升至0.803，LiveCodeBench从0.516提升至0.604。
conclusion: 该方法在不增加额外LLM推理成本的前提下，通过符号化语义分析有效提升了代码生成的推理性能。
---

## 摘要
“Best-of-N”选择是大语言模型（LLM）代码生成中一种流行的推理时扩展方法。然而，为了可靠地识别正确解，现有方法通常依赖于昂贵或随机的外部验证器。在本文中，我们提出了符号等价划分（Symbolic Equivalence Partitioning），这是一个利用符号执行根据语义行为对候选程序进行分组，并从主导功能分区中选择代表性程序的选择框架。为了改进分组和选择，我们在符号执行过程中将特定领域约束编码为可满足性模理论（SMT）假设，以减少路径爆炸并防止在问题领域之外进行无效输入搜索。在 N=10 时，我们的方法在 HumanEval+ 上的平均准确率从 Pass@1 的 0.728 提高到 0.803，在 LiveCodeBench 上从 0.516 提高到 0.604，且除了初始的 N 个候选生成外，不需要任何额外的 LLM 推理。
