...
tldr: 针对边缘设备分布式生成式AI推理中单点故障和不可信行为导致的可靠性问题，本文提出了G-TRAC框架。该框架将路由建模为风险受限最短路径问题，并结合信任层剪枝与Dijkstra算法实现高效路径选择。通过混合信任架构维护全局声誉并同步更新，G-TRAC在异构边缘环境下显著提升了推理完成率和系统鲁棒性。
selection_source: fresh_fetch
motivation: 传统的尽力而为P2P路由无法应对边缘分布式推理中设备失效或恶意行为带来的可靠性挑战。
method: 提出G-TRAC框架，利用风险受限最短路径算法进行路由，并采用混合信任架构同步全局声誉状态。
result: 算法在边缘规模下路由延迟极低（中位数<1ms），并能有效隔离不可信节点，显著提高推理任务的成功率。
conclusion: G-TRAC为动态、异构的边缘分布式AI推理提供了一种兼顾性能与信任的稳健协调方案。
---
