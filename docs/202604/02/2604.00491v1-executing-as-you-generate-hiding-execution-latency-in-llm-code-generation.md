...
tags: ["query:llm"]
score: 7.0
evidence: 在代码生成中隐藏执行延迟
tldr: 本研究针对大语言模型代码生成中“先生成后执行”导致的端到端延迟问题，提出了一种名为Eager的并行执行框架。该框架利用LLM顺序生成代码且不回溯的特性，将生成、检测与执行过程流水线化。通过AST分块、动态批处理和早期错误中断等技术，Eager实现了在代码生成的同时进行执行，显著降低了执行等待时间，在多个基准测试中展现出卓越的加速效果。
selection_source: fresh_fetch
motivation: 传统的串行代码生成与执行模式导致生成器和执行器交替闲置，产生了不必要的端到端延迟。
method: 提出了Eager框架，通过AST分块和动态批处理技术，在代码生成过程中同步进行检测与并行执行。
result: "实验表明Eager可减少高达99.9%的非重叠执行延迟，并将端到端总延迟降低了多达55%。"
conclusion: 这种“边生成边执行”的并行范式能有效隐藏执行延迟，是提升LLM编程智能体效率的有效途径。
---
