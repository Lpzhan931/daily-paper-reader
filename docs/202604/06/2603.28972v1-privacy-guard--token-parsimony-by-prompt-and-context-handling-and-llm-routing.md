...
title_zh: 通过提示词与上下文处理及大语言模型路由实现隐私保护与 Token 节约
...
tldr: "针对大语言模型应用中运营成本与隐私泄露的权衡难题，本文提出“不可分范式”，认为上下文管理与隐私保护本质一致。通过在本地部署小模型构建“隐私卫士”，利用摘要和自动提示词优化技术分解任务，并将高风险查询路由至零信任模型。该方法在实现零泄露的同时，通过减少Token消耗降低了45%的成本，并显著提升了响应质量。"
selection_source: fresh_fetch
motivation: 现有的LLM路由框架在降低成本时忽视了提示词敏感性，导致用户面临向第三方云服务商泄露隐私数据的风险。
method: 提出一种本地“隐私卫士”，利用小模型进行摘要、提示词优化及任务分解，并结合LIFO上下文压缩机制将敏感任务路由至安全模型。
result: "实验显示该框架实现了个人隐私100%脱敏，降低了45%的运营成本，且85%的压缩后响应在评估中优于原始基准。"
conclusion: 研究证明了Token节约与零泄露是上下文压缩算子的两个对偶表现，实现了隐私保护与成本优化的双赢。
---

## 摘要
大语言模型（LLM）的大规模采用迫使人们在运营成本（OpEx）与数据隐私之间进行权衡。目前的路由框架虽然降低了成本，但忽略了提示词的敏感性，使个人用户和机构面临向第三方云提供商泄露数据的风险。我们正式提出了“不可分割范式”（Inseparability Paradigm）：先进的上下文管理在本质上与隐私管理相一致。我们提出了一种本地“隐私卫士”（Privacy Guard）——这是一个由本地小语言模型（SLM）驱动的全方位上下文观察器，它通过抽象式摘要和自动提示词优化（APO）将提示词分解为聚焦的子任务，并将高风险查询重定向至零信任或受保密协议（NDA）保护的模型。这种双重机制在消除敏感推理向量（零泄露）的同时，减少了云端 Token 负载（降低运营成本）。此外，一种基于后进先出（LIFO）的上下文压缩机制进一步限制了工作内存，从而缩小了涌现的泄露面。我们通过一个 2x2 基准测试（懒惰型 vs. 专家型用户；个人 vs. 机构秘密）在 1,000 个样本的数据集上验证了该框架，实现了 45% 的综合运营成本降低，个人秘密脱敏成功率达 100%，且通过“大模型作为裁判”（LLM-as-a-Judge）评估，APO 压缩后的响应相比原始基准获得了 85% 的偏好率。我们的研究结果表明，Token 节约与零泄露是同一上下文压缩算子在数学上的对偶投影。
