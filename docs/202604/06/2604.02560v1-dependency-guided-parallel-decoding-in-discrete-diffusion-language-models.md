...
## 摘要
离散扩散语言模型 (dLLMs) 通过并行取消掩码（unmasking）多个标记（tokens）来加速文本生成。然而，并行解码引入了分布不匹配问题：它使用逐标记边缘分布的全分解乘积来近似联合条件分布，当所选标记之间存在强依赖关系时，这会降低输出质量。我们提出了 DEMASK（依赖引导取消掩码），这是一种附加在 dLLM 最终隐藏状态上的轻量级依赖预测器。在单次前向传递中，它估计被掩码位置之间的成对条件影响。利用这些预测，一种贪心选择算法识别出具有有界累积依赖关系的位置，以便同时取消掩码。在次可加性假设下，我们证明了这限制了并行采样与模型联合分布之间的全变分距离。实验表明，与基于置信度和基于 KL 散度的基准方法相比，DEMASK 在 Dream-7B 上实现了 1.7-2.2 倍的加速，同时保持或提高了准确率。
